...
class XccMarkLogicGetCall(requestName: String, uri: Expression[String], protocol: XccMarkLogicProtocol, val next: ActorRef)
  extends Chainable with DataWriterClient {

  override def execute(session: Session): Unit = {
    val start = TimeHelper.nowMillis
    val result = protocol.callAdhocQuery(s"fn:doc('${uri(session).get}'),xdmp:log('get: ' ||  fn:seconds-from-duration(xdmp:eval('fn:current-dateTime()') - fn:current-dateTime()))")
